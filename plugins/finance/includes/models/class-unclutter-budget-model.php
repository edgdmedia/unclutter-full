 ...
 */
class Unclutter_Budget_Model extends Unclutter_Base_Model
{
     ...
     */
    // Now uses insert, update, delete, get, where from Unclutter_Base_Model
    // Any custom budget-specific queries can be added here
    ...
    public static function get_budget($data)
    {
        ...
        return $wpdb->get_row($wpdb->prepare(
            "SELECT b.*, c.name as category_name 
             FROM $table b 
             LEFT JOIN $categories_table c ON b.category_id = c.id 
             WHERE b.id = %d AND b.profile_id = %d",
            $data['id'],
            $data['profile_id']
        ));
    }
